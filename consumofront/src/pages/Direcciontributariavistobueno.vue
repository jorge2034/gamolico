<template>
  <q-page class="q-pa-xs">
    <q-card>
      <q-badge class="text-h6 full-width text-center" color="secondary" >Direccion tributaria Visto Bueno</q-badge>
...
export default {
  ...
  methods:{
    ...
    vistobueno(i){
      ...
      console.log("id",id)
      this.$q.dialog({
        title:'Seguro de dar VISTO BUENO y enviar a  SECRETARIA MUNICIPAL DE ECONOMIA Y HACIENDA?',
        // message:''
        cancel:true
      }).onOk(()=>{
        this.$q.loading.show()
        // console.log(i)
        this.$axios.post(process.env.API+'/vistobuenotributaria/'+id,{
          dirtributaria:'1',
          estado:'FINALIZADO',
          estado2:'SECRETARIA MUNICIPAL DE ECONOMIA Y HACIENDA',
          nombre:'ENVIADO A SECRETARIA MUNICIPAL DE ECONOMIA Y HACIENDA PARA VISTO BUENO',
          observacion:'VISTO BUENO',
    ...
    mistramites(){
      this.$q.loading.show()
      this.$axios.post(process.env.API+'/mistramitesdtvistobueno').then(res=>{
        console.log(res)
        // console.log(res.data)
        this.tramites=[]
        // res.data.forEach(r=>{
